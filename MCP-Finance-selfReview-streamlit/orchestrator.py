...
async def delegate_to_news_agent(task: str) -> str:
    ...
    - Sector news snapshots (IT, Banking, Energy, Pharma, Auto, FMCG, Defence, Metal, Shipping, etc.)
...
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
CRITICAL STOP RULE — READ FIRST
━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━
Each specialist tool must be called AT MOST ONCE per query.
• TYPE A query  → call delegate_to_news_agent exactly 1 time, then STOP.
• TYPE B query  → call delegate_to_analysis_agent exactly 1 time, then STOP.
• TYPE C/D/E   → call each agent at most 1 time, then STOP.
Once you receive a tool result, DO NOT call ANY tool again.
Immediately write your final synthesised answer.
Calling the same tool twice is an error.
...
[TYPE A] Market briefs, headlines, sector snapshots, general news
  → delegate_to_news_agent ONCE only
  → After receiving the result, STOP calling tools and produce final answer
...
[TYPE C] Sector selection — which sector or stocks to invest in
  Step 1: delegate_to_news_agent      (macro news, sector sentiment at least for 7 days)
  Step 2: delegate_to_analysis_agent  (price trends sector wise, valuation — includes review sector wise)
...
def build_orchestrator() -> object:
    ...
    def orchestrator_node(state: MessagesState) -> dict:
        ...
        # Loop guard: collect names of tools that have already returned results
        completed_tools = {
        ...
        if completed_tools:
            stop_instruction = HumanMessage(
                content=(
                    f"You have already received results from: {', '.join(completed_tools)}. "
                    "DO NOT call any tool again. "
                    "Write your final synthesised answer right now using the template."
                )
            )
...
async def run_orchestrator(query: str) -> str:
    """
    Run the orchestrator for a given query and return the final answer string.
    Designed to be called from Streamlit or other external callers.

    Args:
        query: The user's financial research question.

    Returns:
        The orchestrator's final synthesised answer as a plain string.
    """
...
async def main():
    #task = "Please do the detailed analysis and tell me about Maruti."
    task = "Give me market heqadline and sectors which are performing well today"
    ...
    final = await run_orchestrator(task)
