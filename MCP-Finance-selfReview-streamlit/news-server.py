...
def get_india_business_news(max_results: int = 12) -> str:
    ...
    articles     = _fetch_multiple(
        ["economic_times", "moneycontrol", "financial_express", "business_line", "livemint"],
        limit_each
    )[:max_results]
 
    return json.dumps({
        "category": "Indian Business News",
        "sources":  ["Economic Times", "Moneycontrol", "Financial Express", "Business Line", "livemint"],
        "count":    len(articles),
        "articles": articles,
    }, indent=2)
...
@mcp.tool
def get_sector_news(
    sector: Literal["IT", "Banking", "Energy", "Pharma", "Auto", "FMCG", "Infra", "Telecom", "Defence", "Metal", "Shipping", "Airlines", "Asset Management", "Cement Construction", "Electronics", "Textiles"],
    max_results: int = 8,
) -> str:
    """
